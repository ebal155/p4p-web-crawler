...
class warezbb_data_analyser():
    ...
    def get_movie_counts(self):
        """get all the movie counts"""
        return self.myAnalyser.count_field('movie title',
            return_as_dict=True)
    ...
    def caculate_author_rank_by_average_no_of_views(self):
        """caculates the rank of the authors
        rank 1 = author with most threads
        rank n = author with least threads"""
    ...
    def get_movie_total_replies(self):
        """get all the replies an author has ever had"""
        author_replies = self.myAnalyser.count_two_fields('movie title', 'replies')
        author_total_replies = {}
        for author in author_replies:
            sum = 0
            for view in author_replies[author]:
    ...
    def get_movies_for_author(self):
        ...
        cols =  self.myAnalyser.count_two_fields('author', 'movie title',split_by_comma=False)
        new_cols = {}
    ...
    ########################################
    ########################################
    Printing methods below this point
    ########################################
    ########################################
    ########################################"""
    ...
    def print_author_rank_by_no_of_average_views(self):
        ranks = self.caculate_author_rank_by_average_no_of_views()
        self.my_printer.print_array_to_csv(ranks, "authorRankbyviews.csv")
    ...
    def print_author_rank(self):
        ...
        , 'average_views', "hd", "web", "dvd", "cam", "vhs", "not given","unique content"]
        myArray = self.caculate_author_rank()
        self.my_printer.print_array_to_csv_with_header(myArray,
            header, "author_rank.csv")

    def print_movie_stats(self):
        header = ['movie','total_views','posts']
        movies = self.get_movie_counts()
        total_views = self.get_movie_total_replies()
        myArray = []
        for movie in movies:
            name = movie
            try:
                views = total_views[movie]
            except:
                views = 0
            post_count = movies[movie]
            myArray.append([name, views, post_count])
        return myArray
